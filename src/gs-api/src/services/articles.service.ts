...
class ArticlesService extends __BaseService {
  ...
  findAll(): __Observable<Array<ArticleDto>> {
    return this.findAllResponse().pipe(__map((_r) => _r.body as Array<ArticleDto>));
  }
  ...
  findByCodeArticle(codeArticle: string): __Observable<ArticleDto> {
    return this.findByCodeArticleResponse(codeArticle).pipe(__map((_r) => _r.body as ArticleDto));
  }
  ...
  findById(idArticle: number): __Observable<ArticleDto> {
    return this.findByIdResponse(idArticle).pipe(__map((_r) => _r.body as ArticleDto));
  }
}
