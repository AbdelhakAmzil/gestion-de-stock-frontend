...
class ClientsService extends __BaseService {
  ...
  findAllResponse(): __Observable<__StrictHttpResponse<Array<ClientDto>>> {
    ...
    let __body: any = null;
    let req = new HttpRequest<any>('GET', this.rootUrl + `/gestiondestock/v1/clients/all`, __body, {
      headers: __headers,
      params: __params,
      responseType: 'json',
    });
  ...
  findAll(): __Observable<Array<ClientDto>> {
    return this.findAllResponse().pipe(__map((_r) => _r.body as Array<ClientDto>));
  }
  ...
  findById(idClient: number): __Observable<ClientDto> {
    return this.findByIdResponse(idClient).pipe(__map((_r) => _r.body as ClientDto));
  }
}
