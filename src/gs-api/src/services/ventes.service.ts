...
class VentesService extends __BaseService {
  ...
  findAllResponse(): __Observable<__StrictHttpResponse<Array<VentesDto>>> {
    ...
    let __body: any = null;
    let req = new HttpRequest<any>('GET', this.rootUrl + `/gestiondestock/v1/ventes/all`, __body, {
      headers: __headers,
      params: __params,
      responseType: 'json',
    });
  ...
  findAll(): __Observable<Array<VentesDto>> {
    return this.findAllResponse().pipe(__map((_r) => _r.body as Array<VentesDto>));
  }
  ...
  findByCode(codeVente: string): __Observable<VentesDto> {
    return this.findByCodeResponse(codeVente).pipe(__map((_r) => _r.body as VentesDto));
  }
  ...
  findById(idVente: number): __Observable<VentesDto> {
    return this.findByIdResponse(idVente).pipe(__map((_r) => _r.body as VentesDto));
  }
}
