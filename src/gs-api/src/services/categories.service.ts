...
class CategoriesService extends __BaseService {
  ...
  findAll(): __Observable<Array<CategoryDto>> {
    return this.findAllResponse().pipe(__map((_r) => _r.body as Array<CategoryDto>));
  }
  ...
  findByCode(codeCategory: string): __Observable<CategoryDto> {
    return this.findByCodeResponse(codeCategory).pipe(__map((_r) => _r.body as CategoryDto));
  }
  ...
  findById(idCategory: number): __Observable<CategoryDto> {
    return this.findByIdResponse(idCategory).pipe(__map((_r) => _r.body as CategoryDto));
  }
}
