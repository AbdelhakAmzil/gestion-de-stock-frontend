...
@Component({
  ...
  styleUrls: ['./page-cmd-clt-frs.css'],
})
export class PageCmdCltFrs implements OnInit {
  ...
  findAllCommandes(): void {
    if (this.origin === 'client') {
      this.cmdCltFrsService.findAllCommandesClient().subscribe((cmd) => {
        this.listeCommandes = cmd;
        this.findAllLignesCommande();
      });
    } else if (this.origin === 'fournisseur') {
      this.cmdCltFrsService.findAllCommandesFournisseur().subscribe((cmd) => {
        this.listeCommandes = cmd;
        this.findAllLignesCommande();
      });
    }
