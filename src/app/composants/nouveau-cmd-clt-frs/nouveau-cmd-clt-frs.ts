...
@Component({
  ...
  standalone: false,
  templateUrl: './nouveau-cmd-clt-frs.html',
  styleUrls: ['./nouveau-cmd-clt-frs.css'],
})
export class NouveauCmdCltFrs implements OnInit {
  origin = '';
  ...
  filtrerArticle(): void {
    ...
    this.listArticle = this.listArticle.filter(
      (art) =>
        art.codeArticle?.includes(this.codeArticle) || art.designation?.includes(this.codeArticle),
    );
  }
  ...
  enregistrerCommande(): void {
    ...
    if (this.origin === 'client') {
      ...
      this.cmdCltFrsService
        .enregistrerCommandeFournisseur(commande as CommandeFournisseurDto)
        .subscribe(
          (cmd) => {
